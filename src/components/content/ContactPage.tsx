...
export default function ContactPage({ contactData }: { contactData: Contact[] }) {
    ...
    return (
        <section className='bg-[#fff7e6] pt-8 container' id='kontak'>
            ...
            <div className="bg-[#e8d1b7] px-10 py-10 md:py-20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {contactData.map((contact, idx) => (
                        <Card key={idx} className="bg-[#f7f1e6] border-none shadow-none px-0 py-6">
                            <CardContent className="flex flex-col items-start gap-2">
                                ...
                                <Link href={contact.url} target='_blank' rel='noopener noreferrer' className="text-[#e07a5f] text-sm font-medium hover:underline">{contact.label}</Link>
                            </CardContent>
                        </Card>
                    ))}
                </div>
